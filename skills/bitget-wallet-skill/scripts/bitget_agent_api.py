...
def get_market_rank(chain: str, limit: int = 20) -> dict:
    """
    Get token rankings/trending tokens.
    """
    body = {
        "chain": chain,
        "limit": limit,
    ...
    # Using the same signing mechanism for market data if available on this endpoint
    # Or falling back to public endpoint if different
    return _request("/swap-go/market/rank", body)
