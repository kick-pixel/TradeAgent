...
def main():
    ...
    parser.add_argument(
        "--private-key", default=None, help="EVM private key (hex, from secure storage)"
    )
    parser.add_argument(
        "--private-key-sol",
        default=None,
        help="Solana private key (base58 or hex, from secure storage)",
    )
    parser.add_argument("--from-address", required=True, help="Sender address")
    parser.add_argument(
        "--to-address", required=True, help="Receiver address (usually same as from-address)"
    )
    ...
    if not args.private_key and not args.private_key_sol:
        print(
            "Error: must provide --private-key (EVM) or --private-key-sol (Solana)", file=sys.stderr
        )
        sys.exit(1)
    ...
    if _is_solana_order(data):
        if not args.private_key_sol:
            print(
                "Error: Solana order detected but --private-key-sol not provided", file=sys.stderr
            )
            sys.exit(1)
